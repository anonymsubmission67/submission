...
with open('data/claim_preporcessing/politifact_scraped.jsonl', 'r') as f: # before: politifact_scraped_new.jsonl
...
df = df[(df["year"] > 2018) & (df["year"] < 2023)]  # df[df["year"] > 2022 ] 

df.to_csv('data/claim_preporcessing/politifact_processed_2019_2022.csv', index=False)

print(f"\nTotal claims removed due to image/video references: {removed_count}")
print(f"After filtering for year: {len(df)}")
