"""
Build correlation plots for no-persona and persona data.
...
def _load_overview_data() -> pd.DataFrame:
    ...
    Returns:
        DataFrame with overview metrics
    """
...
def _create_correlation_heatmap(metrics: pd.DataFrame, ax, title: str = ""):
    """
    Create a triangular correlation heatmap.
    
    Args:
        metrics: DataFrame with metrics to correlate
        ax: Matplotlib axis
    ...
    corr_subset = corr_matrix.drop('f1_2', axis=1).drop("pol_bias", axis=0)
    ...
    for i in range(len(corr_subset.index)):
        for j in range(len(corr_subset.columns)):
            if not pd.isna(corr_subset.iloc[i, j]):
                ax.text(j, i, f'{corr_subset.iloc[i, j]:.3f}',
...
def build_correlation_plot():
    """
    Create correlation heatmaps for no-persona and persona data.
    
    Generates two separate triangular correlation heatmaps showing
    relationships between metrics for both conditions.
    """
    # Load data
    ...
    # Filter to LLMs with complete data
    required_cols = ["pol_bias", "llm_size", "me_diff_1", "f1_1", "me_diff_2", "f1_2"]
    llms_no_persona = set(
        no_persona_data[no_persona_data[required_cols].notna().all(axis=1)]["llm"]
    )
    llms_persona = set(
        persona_data[persona_data[required_cols].notna().all(axis=1)]["llm"]
    )
    ...
    # Filter to common LLMs
    no_persona_metrics = no_persona_data[
        no_persona_data["llm"].isin(common_llms)
    ][required_cols]
    persona_metrics = persona_data[
        persona_data["llm"].isin(common_llms)
    ][required_cols]
    ...
    if len(no_persona_metrics) > 1:
        ...
        _create_correlation_heatmap(no_persona_metrics, ax)
        plt.tight_layout()
    ...
    if len(persona_metrics) > 1:
        fig, ax = plt.subplots(1, 1, figsize=(10, 5))
        _create_correlation_heatmap(persona_metrics, ax)
        plt.tight_layout()
