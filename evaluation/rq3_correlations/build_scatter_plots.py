"""
Build scatter plots showing correlations between PB-A and PB-T differences.
"""
...
def _calculate_point_size(llm: str) -> float:
    """
    Calculate point size based on LLM size.
    ...
    """
    llm_size = llm_info.get(llm, {}).get("size_B", 8)  # Default to 8B
    return max(POINT_SIZE_MIN, min(POINT_SIZE_MAX, llm_size * POINT_SIZE_MULTIPLIER))


def _plot_scatter_points(ax, scatter_data: pd.DataFrame, x_col: str, y_col: str):
    """
    Plot scatter points for LLMs.
    
    Args:
        ax: Matplotlib axis
        scatter_data: DataFrame with scatter plot data
        x_col: Column name for x-axis
        y_col: Column name for y-axis
    """
    unique_llms = scatter_data['llm'].unique()
    for llm in unique_llms:
        llm_data = scatter_data[scatter_data['llm'] == llm]
        color_info = llm_colors.get(llm, {"edgecolor": "gray"})
        color = color_info["edgecolor"]
        point_size = _calculate_point_size(llm)
        
        ax.scatter(
            llm_data[x_col], llm_data[y_col],
            c=color, s=point_size, edgecolors='black', linewidth=0.5
        )


def _add_correlation_text(ax, scatter_data: pd.DataFrame, x_col: str, y_col: str):
    """
    ...
    Args:
        ...
        y_col: Column name for y-axis
    """
    corr_coef = scatter_data[x_col].corr(scatter_data[y_col])
    ax.text(0.025, 0.90, f'r = {corr_coef:.3f}', transform=ax.transAxes,
           fontsize=12, bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))


def _create_legend_models(ax, scatter_data: pd.DataFrame, bbox_to_anchor: tuple):
    """
    ...
    Args:
        ax: Matplotlib axis
        scatter_data: DataFrame with scatter plot data
        bbox_to_anchor: Position for legend
    """
    models_legend_entries = []
    for llm in target_llms + opinion_llms:
        if llm in scatter_data['llm'].values:
    ...
    return plt.legend(handles=models_legend_entries, loc='center right',
                     bbox_to_anchor=bbox_to_anchor, title="Models",
                     framealpha=1, facecolor="white",
                     columnspacing=1.0, handletextpad=0.5)


def _create_legend_sizes(ax, bbox_to_anchor: tuple):
    """
    ...
    for size_b in size_examples:
        point_size = max(POINT_SIZE_MIN, min(POINT_SIZE_MAX, size_b * POINT_SIZE_MULTIPLIER))
        size_legend_entries.append(
            mlines.Line2D([], [], color='gray', marker='o',
                         linestyle='None', markersize=np.sqrt(point_size)/2,
                         label=f'{size_b}B                     ')
        )
    
    return plt.legend(handles=size_legend_entries, loc='lower right',
                     bbox_to_anchor=bbox_to_anchor, title="Model Size",
...
def build_scatter_plots():
    """
    Create scatter plots showing correlations between political bias and ME differences.
    
    Generates:
    1. No-persona scatter plots (two subplots: party-agnostic and party-aware)
    2. Persona scatter plot with political view symbols
    """
    # Load data
    ...
    print(f"No Persona data shape: {no_persona_scatter_1.shape}")
    print(f"Persona data shape: {persona_scatter.shape}")
    
    # Calculate common axis scales
    ...
    # Plot 1: No Persona - Two scatter plots
    if len(no_persona_scatter_1) > 0 and len(no_persona_scatter_2) > 0:
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(6, 5))
        fig.subplots_adjust(right=0.65, top=0.98, bottom=0.1, hspace=0.2)
        
        # Plot Prompt 1
        _plot_scatter_points(ax1, no_persona_scatter_1, 'pol_bias', 'me_diff_1')
        ax1.set_ylabel('PB-T party-agnostic', fontsize=12)
        ax1.grid(True, alpha=0.3)
        ax1.set_xlim(x_min, x_max)
        ax1.set_ylim(y_min, y_max)
        _add_correlation_text(ax1, no_persona_scatter_1, 'pol_bias', 'me_diff_1')
        
        # Plot Prompt 2
        _plot_scatter_points(ax2, no_persona_scatter_2, 'pol_bias', 'me_diff_2')
        ax2.set_xlabel('PB-A', fontsize=12)
        ax2.set_ylabel('PB-T party-aware', fontsize=12)
        ax2.grid(True, alpha=0.3)
        ax2.set_xlim(x_min, x_max)
        ax2.set_ylim(y_min, y_max)
        _add_correlation_text(ax2, no_persona_scatter_2, 'pol_bias', 'me_diff_2')
        
        # Add legends
        legend1 = _create_legend_models(ax1, no_persona_scatter_2, (1.65, 0.15))
        ax1.add_artist(legend1)
        _create_legend_sizes(ax2, (1.647, -0.10))
        
        output_path = output_dir / "correlation_no_persona.png"
        plt.savefig(output_path, dpi=300)
        print(f"No Persona scatter plots saved to: {output_path}")
    else:
        print("Insufficient data for No Persona plots")
    
    # Plot 2: Persona - Scatter plot with symbols
    if len(persona_scatter) > 0:
        ...
        for llm in unique_llms:
            ...
            for _, row in llm_data.iterrows():
                persona_data = df[
                    (df['llm'] == llm) &
                    (df['pol_bias'] == row['pol_bias']) &
                    (df['me_diff_2'] == row['me_diff_2'])
                ]
        ...
        # Set axis limits (use common limits if available)
        if len(no_persona_scatter_2) > 0:
        ...
        models_legend_entries = []
        for llm in target_llms + opinion_llms:
            if llm in persona_scatter['llm'].values:
                llm_name = llm_info.get(llm, {}).get("name", llm.capitalize())
                models_legend_entries.append(
                    mlines.Line2D([], [], color=llm_colors[llm]["edgecolor"], marker='o',
                                 linestyle='None', markersize=5, label=llm_name)
                )
        ...
        legend1 = plt.legend(handles=models_legend_entries, loc='center left',
                            bbox_to_anchor=(1.02, 0.59), title="Models",
                            framealpha=0.8, facecolor="white",
                            columnspacing=1.0, handletextpad=0.5, fontsize=7, title_fontsize=8)
        plt.gca().add_artist(legend1)
        plt.legend(handles=political_legend_entries, loc='center left',
                  bbox_to_anchor=(1.02, 0.06), title="Political Viewpoints",
                  framealpha=0.8, facecolor="white",
                  columnspacing=1.0, handletextpad=0.4, fontsize=7, title_fontsize=8)
        
        output_path = output_dir / "correlation_persona.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Persona scatter plot saved to: {output_path}")
    else:
        print("Insufficient data for Persona plots")
    
    # Print summary
    if len(no_persona_scatter_2) > 0 and len(persona_scatter) > 0:
        print(f"\nSummary:")
        print(f"No Persona - Correlation: {no_persona_scatter_2['pol_bias'].corr(no_persona_scatter_2['me_diff_2']):.3f}")
        print(f"Persona - Correlation: {persona_scatter['pol_bias'].corr(persona_scatter['me_diff_2']):.3f}")
        print(f"No Persona data points: {len(no_persona_scatter_2)}")
