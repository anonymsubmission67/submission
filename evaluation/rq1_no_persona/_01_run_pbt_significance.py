...
from utils import target_llms, llm_colors, llm_info, CLAIM_MATRIX_ROOT, pbt_mean_path

def run_pbt_significance():
    ...
    mx_root = CLAIM_MATRIX_ROOT

    print(f"\n{'='*80}")
    print(f"Processing pooled claim matrices under {CLAIM_MATRIX_ROOT}")
    print(f"{'='*80}\n")
    ...
    for prompt in ["1", "2"]:
        ...
        try:
            ...
            print(f"Warning: Could not find pooled matrix for prompt {prompt}: {e}")
            continue
        ...
        for llm in target_llms:
            ...
            for axis in ["economic", "social", "both"]:
                ...
                for metric in ["me", "mae"]:
                    ...
                    if axis == "both":
                        rep_data = df_long[
                            (df_long["party"] == "Republican")
                            & (df_long["axis"].isin(["economic", "social"]))
                        ][col_name].dropna()
                        dem_data = df_long[
                            (df_long["party"] == "Democrat")
                            & (df_long["axis"].isin(["economic", "social"]))
                        ][col_name].dropna()
                    else:
                        rep_data = df_long[
    ...
    if len(test_summary_data) == 0:
        ...
        print("Warning: No pooled PBT data processed. Creating empty summary file.")
    else:
        test_summary_df = pd.DataFrame(test_summary_data)

    output_path = out_dir / "shapiro_wilk_summary_pooled.csv"
