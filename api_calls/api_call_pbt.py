...
def flip_claim_polview_for_prompt_3(party: str) -> str:
    """Same template as prompt_2, but invert Democrat/Republican for the wording."""
    if party == "Democrat":
        return "Republican"
    if party == "Republican":
        return "Democrat"
...
def make_user_prompt(claim_text, claim_polview, prompt_type, prompt_style="simple"):
    ...
    Args:
        ...
        prompt_type: 0 for prompt_1, 1 for prompt_2
        prompt_style: "simple" or "chain_of_thought"
    """
    prompt_name = f"prompt_{prompt_type + 1}"
...
def parse_args():
    ...
    parser.add_argument("--prompt", choices=["prompt_1", "prompt_2", "prompt_3"], required=True,
                        help="Prompt variant to run (outputs under prompt_N/; prompt_3 flips party vs metadata)")
    ...
    parser.add_argument("--prompt-type", type=int, choices=[0, 1], default=None,
                        help="Override template: 0=promp_1, 1=prompt_2 "
                             "(default: prompt_1→0, prompt_2→1, prompt_3→1)")
    ...
    parser.add_argument("--claim-start", type=int, default=0, help="0-based index: skip first N claims (e.g. 259 = start at 260th claim)")
...
def main():
    ...
    base_dir = Path("data/api_outputs") / args.prompt_style / "pbt"
    ...
    if args.claim_start > 0:
        claims = claims[args.claim_start:]
        print(f"Starting from claim index {args.claim_start} ({len(claims)} claims)")

    default_map = {"prompt_1": 0, "prompt_2": 1, "prompt_3": 1}
    ...
    # Wenn ab anderem Claim gestartet wird: anderes Output-File, damit bestehende Runs nicht überschrieben werden
    file_suffix = f"_from{args.claim_start}" if args.claim_start > 0 else ""

    for run_idx in range(run_start, run_end + 1):
        # One file per run: baseline_resultsXX.jsonl (mit _from259 wenn --claim-start 259)
        pad = max(2, len(str(total_runs)))
        run_tag = f"{run_idx:0{pad}d}"
        output_path = prompt_dir / f"baseline_results{run_tag}{file_suffix}.jsonl"
        ...
        with output_path.open("a", encoding="utf-8") as outfile:
            for index, claim in enumerate(pending_claims, start=1):
                ...
                if args.prompt == "prompt_3":
                    claim_polview = flip_claim_polview_for_prompt_3(claim_polview)
