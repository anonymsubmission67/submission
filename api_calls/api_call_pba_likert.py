...
def make_user_prompt(claim_text, prompt_style="simple"):
    ...
    """
    prompt_name = "prompt_questionnaire"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate questionnaire responses (no persona baseline, repeated runs)")
    parser.add_argument(
        "--questionnaire",
        choices=["coordinates", "compass"],
        default="coordinates",
        help="Which CSV to use: data/political_{name}_statements.csv (default: coordinates)",
    )
...
def main():
    args = parse_args()
    q = args.questionnaire

    base_dir = Path("data/api_outputs") / args.prompt_style / q
    model_dir_name = resolve_model_dir(args.model)
    model_dir = base_dir / model_dir_name
    prompt_dir = model_dir
    prompt_dir.mkdir(parents=True, exist_ok=True)

    # Load questionnaire statements
    claims_path = Path("data") / f"political_{q}_statements.csv"
    with claims_path.open("r", newline="", encoding="utf-8") as claim_file:
    ...
    for run_idx in range(run_start, run_end + 1):
        # One file per run: baseline_{questionnaire}{run_tag}.jsonl
        pad = max(2, len(str(total_runs)))
        run_tag = f"{run_idx:0{pad}d}"
        output_path = prompt_dir / f"baseline_{q}{run_tag}.jsonl"
